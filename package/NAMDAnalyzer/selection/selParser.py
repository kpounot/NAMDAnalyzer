...
class SelParser:
    """ This class is used to parse a simple selection text, calls the
        appropriate functions from :class:`.NAMDPSF` and :class:`.NAMDDCD`.

        A dataContext is provided to the class, that is simply the
        :class:`.Dataset` class, in which psf and dcd files are loaded.

        Atom indices are stored for each frame in *selection* attribute.

        Examples:

        To select hydrogens bound to water oxygens within 3 angstroms
        of a protein region for a given frame range, use from class
        :class:`.Dataset` instance d:
    ...
    def __init__(self, dataContext, selT=None):
        ...
        # Selections that will be compared using 'and' operator
        self.and_selList = []
    ...
    def process(self):
        ...
        if type(self.selT) != str:
            print('Selection text should be a string instance, '
                  'the given argument cannot be parsed.')
            return


        # Parses frame keyword
    ...
    def _parseText(self, withinList=None):
        ...
        if re.search('same resid as', self.selT):
            sel.append(self._getSameResid(
                self.selT[self.selT.find('same resid as'):], withinList))
            withinList = None

        if re.search('bound to', self.selT):
            sel.append(self._getBoundTo(
                self.selT[self.selT.find('bound to'):], withinList))
            withinList = None
    ...
    def _getSelection(self, partialText):
        """ This method is used by parseText to generate the different
            independent selections from selection text before these are
            compared using 'and' operators to generate the final selection.
        ...
        for selCmd in partialText:
            ...
            if selCmd != '':
                ...
                if np.isin(selCmd, self.keywords):
                    selList.append(
                        self.dataContext.getSelection(selCmd, invert=invert))
                    continue
                ...
                selList.append(
                    self.dataContext.getSelection(**self.selKwdDict,
                                                  invert=invert))
    ...
    def _getSameResid(self, partialSel, withinList=None):
        ...
        if re.search('bound to', partialSel):
            sel.append(self._getBoundTo(
                partialSel[partialSel.find('bound to'):], self.withinList))
            ...
            if partialSel != '':
                sel.append(np.intersect1d(
                    sel[0], self._getSelection(partialSel)))
    ...
    def _getWithin(self, partialSel):
        """ Used specifically to parse within keyword selection.
            The 'within' part is processed and the rest of the
            selection text is returned.
        ...
        if re.search('same resid as', partialSel):
            sel = self._getSameResid(
                partialSel[partialSel.find('same resid as'):])
            ...
            sel = self._getBoundTo(
                partialSel[partialSel.find('bound to'):])
            partialSel  = partialSel[:partialSel.find('bound to')]
        ...
        if outSel != '':
            ...
            if outSel[-1] == '':
                sel = self.dataContext.getWithin(
                    distance, sel, frame=self.frame)

            elif re.search('same resid as', outSel[-1]):
                if outSel[-1].split(' as ')[-1] != '':
                    sel = self.dataContext.getWithin(
                        distance, sel, outSel[-1].split('as')[-1],
                        frame=self.frame)
                else:
                    sel = self.dataContext.getWithin(
                        distance, sel, frame=self.frame)
                ...
                if outSel[-1].split(' to ')[-1] != '':
                    sel = self.dataContext.getWithin(
                        distance, sel, outSel[-1].split('to')[-1],
                        frame=self.frame)
                else:
                    sel = self.dataContext.getWithin(
                        distance, sel, frame=self.frame)
                ...
                sel = self.dataContext.getWithin(
                    distance, sel, outSel[-1], frame=self.frame)
    ...
    def _getBoundTo(self, partialSel, withinList=None):
        ...
        if re.search('same resid as', partialSel):
            sel.append(self._getSameResid(
                partialSel[partialSel.find('same resid as'):], withinList))
            partialSel  = partialSel[:partialSel.find('same resid as')]
            ...
            if partialSel != '':
                sel.append(np.intersect1d(sel[0],
                           self._getSelection(partialSel)))
