...
class Rotations:
    """ This class defines methods to compute rotational relaxation and
        orientation probabilities.

        Some plotting methods are also available to quicly check the results.

        :arg data:        a Dataset class instance containing
                          trajectories data
        :arg sel1:        first selection corresponding to one end of
                          each vector
        :arg sel2:        second selection for vectors, should be of same
                          size as sel1
        :arg tMax:        maximum number of frames to be used
        :arg step:        time interval between each computed vectors
        :arg dPhi:        angular bin size for orientational probability
                          (in degrees)
    ...
    def __init__(self, data, sel1, sel2, tMax=100, step=1,
                 dPhi=0.5, axis='z', nbrTimeOri=20):
...
# --------------------------------------------
    def compRotationalRelaxation(self):
        """ For each frame in the range of tMax with gieven step,
            computes the distance vector between sel1 and sel2 and
            performs scalar product between the first one and all other one.
            This is averaged over multiple time origins and the obtained
            correlation is stored in self.rotRelax variable.

            References:

                - Yu-ling Yeh and Chung-Yuan Mou (1999).
                  Orientational Relaxation Dynamics of Liquid Water
        ...
        for idx, frame in enumerate(oriList):
            ...
            sel1 = self.data.dcdData[self.data.selection(sel1),
                                     frame:frame + self.tMax:self.step]
            sel2 = self.data.dcdData[self.data.selection(sel2),
                                     frame:frame + self.tMax:self.step]

            angles = sel2 - sel1
            angles = angles / np.sqrt(
                np.sum(angles**2, axis=2))[:, :, np.newaxis]
    ...
    def compOrientationalProb(self):
        """ Compute the probability for the vector between sel1
            and sel2 to be in a particular orientation.
            The angle is computed with respect to a given axis.
            Averaging is performed for each frame between 0 and
            tMax with given step.
...
class WaterAtProtSurface:
    """ This class provides way to compute water molecules orientation
        relative to protein surface.

        Basically, for each selected water molecule, approximately 3-7
        nearest atoms from protein are selected, their geometric center
        is computed and the vector between water oxygen and this center
        defines the normal to protein surface.
        The dipole moment vector of water molecule is dotted with this
        normal vector to determine the orientation.
        ...
        :arg minR:    minimum distance from protein surface for
                      water molecules selection
        :arg maxR:    maximum distance from protein surface for
                      water molecules selection
        :arg maxN:    maximum number of protein atoms to be used to compute
                      normal to surface
        :arg frames:  frames to be used to average orientations
        :arg watVec:  vector on water molecule to be used to compute
                      orientation, can be 'D' for electric dipole moment,
                      'H1' for O-H1 vector or 'H2' for O-H2 vetor.
        :arg nbrVox:  number of voxels to be used in each dimensions to
                      generate the volumetric map.
        :arg gaussianFiltSize: possibility to use a gaussian filter on
                               volumetric map. This calls the
                               ``scipy.ndimage.gaussian_filter`` method with
                               default parameter and this argument given for
                               *sigma* argument. Set it to 1 to not use it.
    ...
    def compOrientations(self):
        """ Computes, for all selected water oxygens, the orientation of
            the dipole moment vector relative to protein surface.

            This generates a volumetric dataset of size
            *nbrVox x nbrVox x nbrVox*, in which each voxel
        ...
        self.orientations = np.ascontiguousarray(waterO[:, :, 0],
                                                 dtype='float32')
        self.keepWat      = np.ascontiguousarray(waterO[:, :, 1],
                                                 dtype=bool)
    ...
    def generateVolMap(self, align=False):
        """ Generates a volumetric map with water orientations relative
            ...
            First, if align keyword is set to True, protein structures
            are aligned for all frames. Else, only center of mass are aligned.

            Then water molecules are moved based on periodic boundary
            conditions applied on distance between water oxygen and closest
            protein atom, such that the volumetric map corresponds to the
            positions computed with :py:meth:compOrientations .

            Eventually, for each voxel, orientation of water molecules present
            in the voxel limits is averaged over all frames.

            Using :py:meth:`writeVolMap`, a .pdb file and a .dx file are
            created and can be directly imported into VMD to visualize average
            orientation for each voxel.
    ...
    def writeVolMap(self, fileName=None, frame=0, pFrames=None):
        """ Write the volumetric map containing averaged water orientations
            ...
            The file is in the APBS .dx format style, so that it can be
            imported directly into VMD. Moreover, a pdb file is also generated
            containing frame averaged coordinates for aligned protein.

            :arg fileName: file name for .pdb and .dx files. If none, the name
                           of the loaded .psf file is used.
            :arg frame:    frame to be used to generate the pdb file
            :arg pFrames:  if not None, this will be used instead for protein
                           frame selection and the resulting coordinates will
                           be an average over all selected frames
        ...
        wSel = self.data.selection(
            wSel._indices[np.argwhere(toKeep.astype(bool))[:, 0]])
    ...
    def getOrientations(self, nbrBins=100, frames=None):
        """ Computes the distribution of orientations between -1 and 1
            given the number of nbrBins and for given frames.

            :arg nbrBins: number of bins to use
            :arg frames:  frames to be used
                          (either None for all frames, or slice/range/list)

            :returns: a tuple containing the bin edges and the
                      orientation density
    ...
    def getAutoCorr(self, dr=0.2, nbrBins=100):
        """ Computes the autocorrelation function of water orientation
            distribution as a function of the distance r.
            The bins are given by taking the range between minR and maxR
            with step dr. The distributions :math:`\\rho(r)` are computed
            for all waters whose distance lie within the given bin edges.
            Then autocorrelation is computed as:

            .. math::

                C(\\Delta r) = \\frac{\\sum_{angles} \\rho(r_{0})
                                      \\rho(r_{0} + \\Delta r)}
                                     {\\sum_{angles} \\rho(r_{0})\\rho(r_{0})}

            :arg dr:   distance step to be used between each distribution
            :math:`\\rho(r)` :arg nbrBins: number of bins to use to compute
            orientation distributions

            :returns: a tuple containing an array of bin edges,
                      and an array of correlations

        """

        dist = np.stack((self.distances.flatten(),
    ...
    def getDistEntropy(self, dr=0.2, nbrBins=100):
        """ Computes the entropy of water orientation distribution as a
            function of the distance r. The bins are given by taking the
            range between minR and maxR with step dr. The distributions
            :math:`\\rho(r)` are computed for all waters whose distance
            lie within the given bin edges. Then entropy is computed using
            scipy *entropy* routine.

            :arg dr:      distance step to be used between each
                          distribution :math:`\\rho(r)`
            :arg nbrBins: number of bins to use to compute orientation
                          distributions

            :returns: a tuple containing an array of bin edges,
                      and an array of entropies
...
# Plotting methods
# --------------------------------------------
    def plotOrientations(self, nbrBins=100, frames=None,
                         kwargs={'lw': 1}):
        """ Plots orientations of water molecule, within the range
            (minR, maxR) for given frame.
    ...
    def plotDistEntropy(self, dr=0.2, nbrBins=100):
        """ Plot the entropy of water orientation distribution as a
            ...
            This calls the :py:meth:`getDistEntropy`
