...
class ResidenceTime:
    """ This class defines methods to compute retention time of atoms
        in a certain region. This determines how fast atoms in sel2 can
        leave the vicinity of sel1.

        Some plotting methods are also available to quicly check the results.

        :arg data:       a :class:`Dataset` class instance containing
                         trajectories data
        :arg sel:        selection corresponding to analysis region
                         (with 'within' keyword')
...
# --------------------------------------------
    def compResidenceTime(self):
        """ For each frame in the range of tMax with given step,
            the number of atoms in selected region is computed and divided
            by the number of atoms at time origin. The result is averaged over
        ...
        for idx, frame in enumerate(oriList):
            ...
            sel = self.data.selection(
                self.sel + " frame %i:%i:%i"
    ...
    def compResidueWiseResidenceTime(self, dt=25):
        """ Computes, for a given time dt, the residence time for
            ...
            If different proteins are present, the *segName* argument
            can be used in selection text.

            :arg dt: time step to compute residence time. Basically,
                     the number of selected molecules wihtin the given
                     region at initial time divides the number that stayed
                     within the region after a time dt (in number of frames).
        ...
        for resId, residue in enumerate(resSel):
            for idx, frame in enumerate(oriList):
                print("Processing time origin %i of %i for "
                      "residue %i of %i...        "
                      % (idx + 1, oriList.size, resId + 1,
                         int(resSel.size)), end='\r')

                # Processing selection text
                selText = self.sel
                ...
                sel = self.data.selection(
                    selText + " frame %i %i" % (frame, frame + dt))
...
# --------------------------------------------
    ...
    def plotResidueWiseResidenceTime(self):
        ...
        ax.set_ylabel('P(dt=%i ps)'
                      % np.ceil(self.residueWise_dt
                                * self.data.timestep
                                * self.data.dcdFreq[0] * 1e12))
