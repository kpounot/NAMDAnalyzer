...
class WaterVolNumberDensity:
    """ This class defines methods to compute volumetric map of number density of selected atoms.
        Additional methods to plot a distribution and related entropy are available.
        
        :arg data:    a :class:`Dataset` class instance containing trajectories data 
        :arg protSel: selection of protein atoms to be used for calculations
        :arg watSel:  water selection for which number of atoms in each voxel will be computed for 
                      each selected frame. If *within* keyword is used, it should be preceded by a
    ...
    def __init__(self, data, protSel='protein', watSel='water', frames=None, nbrVox=32):
    ...
    def generateVolMap(self, align=False):
        """ This method computes, for each frame the number density of selected atom in each voxel
            of the volumetric map.

            :arg nbrBins: number of bins to be used to compute the histograms
            :arg align:   if True, all atoms will be aligned for each frame, taking the first frame
                          as reference set. Else, only center of mass of selected atoms are aligned.
        ...
        volMap = np.zeros( (self.nbrVox, self.nbrVox, self.nbrVox), dtype='float32')
    ...
    def getDistribution(self, nbrBins=100, minN=0):
        """ Computes the ditribution of water molecule number in volumetric map. 
            ...
            :arg minN:    minimum limit on volumetric map values, only voxels containing values higher 
                          than given one will be used.
        
        """

        volMap = self.volMap[ self.volMap > minN ]

        edges = np.arange( volMap.min(), volMap.max(), (volMap.max() - volMap.min()) / nbrBins,
                           dtype='float32' )
...
#---------------------------------------------
    def plotDistribution(self, nbrBins=100, minN=0, kwargs={'lw':1}):
        """ Plots the distribution of density for all voxel having a value higher than 0. 
            ...
            :arg frames: frames to be used in *histList* attribute for plotting, if multiple frames are
                         selected, the average is returned.
                         This argument should be an integer, a list or a range.
            :arg minN:   minimum limit on volumetric map values, only voxels containing values higher 
                         than given one will be used.
