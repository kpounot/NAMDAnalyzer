...
class ScatDiffusion(HydroproReader):
    """ This class defines methods to compute corrected diffusion coefficients.
        Starting from coefficients Dt0, for translational diffusion and Dr0
        for rotational diffusion in the dilute limit obtained with HYDROPRO
        for example, corrections on the volume fraction and coefficient
        themselves can be performed to obtain the apparent diffusion
        coefficient for crowded solution that can be used to compare with
        experimental scattering data.

        :arg dataset:      a self.dataset class instance containing
                           trajectories data
        :arg hydroproFile: output .res file from hydropro containing
                           computes parameters
...
# --------------------------------------------
    def compEffVolFrac(self, massP, solventVol=1, compSpecVol=False,
        ...
        """ Computes the hydrodynamic radius based on translational diffusion
            coefficient in the dilute limit and uses the relation,
            effPhi = phi * (Rh/R)**3 to get the effective volume fraction.

            :arg massP:       mass of protein used in the experiment
                              (in grams)
            :arg solventVol:  volume of solvent used (in cm^3), to compute
                              initial volume fraction
            :arg compSpecVol: if True, computes the specific volume based in
                              protein sequence instead of using the one from
                              HYDROPRO.
            :arg selection:   if compSpecVol is True, use the given selection
                              to compute the specific volume
            :arg frame:       if compSpecVol is True, use the given frame to
    ...
    def _compCorrectedDt(self):
        """ Computes the translational diffusion coefficient in crowded
            solution based on protein's effective volume fraction phi and
            translational diffusion coefficient Ds0 in the dilute limit.
    ...
    def compAppDiffCoeff(self, qVals, maxN=100, density_dr=0.5,
        ...
        """ Computes apparent diffusion coefficient D based on corrected
            diffusion coefficients Dt and Dr.

            As Dt and Dr are in [cm] and [s] units, the obtained result
            is given in cm^2/s

            :arg qVals:       a list of scattering vector amplitude values
                              to compute D (in [angström])
            :arg maxN:        maximum number of spherical bessel function
                              to be used
            :arg density_dr:  increment value for the radius r used to compute
                              the radial density from the protein
                              center-of-mass
            :arg maxDensityR: maximum radius to be used to compute
                              the radial density
            :arg frame:       frame to be used on the loaded trajectory to
        ...
        density = RadialNumberDensity(self.dataset, 'protH', 'protH',
                                      dr=density_dr,
    ...
    def compDt(self, qVals, D, Dr, maxN=100, density_dr=1, maxDensityR=60,
        ...
        """ Computes coefficients Dt based on the apparent diffusion
            ...
            :arg qVals:       a list of scattering vector amplitude values to
            ...
            :arg density_dr:  increment value for the radius r used to compute
                              the radial density from the protein
                              center-of-mass
            :arg maxDensityR: maximum radius to be used to compute the
                              radial density
            :arg frame:       frame to be used on the loaded trajectory to
                              compute the radial density
